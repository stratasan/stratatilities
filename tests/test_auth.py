    ...
    with patch(path + "request_vault_token") as request_vault:
        with patch(path + "hvac") as hvac:
            ...
            hvac.Client.assert_called_with(
                url=url, verify=False, token=request_vault.return_value
            )
...
def test_read_vault_secret():
    ...
    client.read.return_value = {"data": {"value": "output"}}
    ...
    complex_value = {"complex": "values", "another": "key"}
    client.read.return_value = {"data": complex_value}
...
def test_get_vault_client_via_ldap():
    with patch("stratatilities.auth.hvac") as hvac, patch(
        "stratatilities.auth.getpass"
    ) as getpass:
        username, vault_addr = "username", "https://vault.example.com"

        hvac.Client.return_value.is_authenticated.return_value = True

        client = get_vault_client_via_ldap(username, vault_addr=vault_addr)
        ...
        client.auth.ldap.login.assert_called_with(
            username=username, password=getpass.getpass.return_value, mount_point="ldap"
        )
